...
export const YawPitchOffset = () => {
    ...
    const [yffset, setYOffset] = useState(0)
    ...
    function writeOutput() {
        ...
        let outputArray = [
            `%math_{player_x_long} + (${zOffset} * cos({player_pitch}*-1) * sin({player_yaw}*-1)) + (1 * cos({player_yaw}*-1))%`,
            `%math_{player_y_long} + (${zOffset} * sin({player_pitch}*-1)) + (2)%`,
            `%math_{player_z_long} + (${zOffset} * cos({player_pitch}*-1) * cos({player_yaw}*-1)) - (1 * sin({player_yaw}*-1))%`
        ]
    }
